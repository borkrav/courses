...
extern "C"
int gr_cyllinder_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  const char* name = luaL_checkstring(L, 1);
  data->node = new GeometryNode(name, new Cyllinder());
...
int gr_light_cmd(lua_State* L)
{
  ...
  get_tuple(L, 3, l.falloff, 3);
	get_tuple(L, 4, l.area, 2);
	get_tuple(L, 5, norm, 3);
	get_tuple(L, 6, left, 3);

  l.colour = Colour(col[0], col[1], col[2]);
	l.normal = Vector3D(norm[0], norm[1], norm[2]);
	l.left = Vector3D(left[0], left[1], left[2]);
...
int gr_render_cmd(lua_State* L)
{
  ...
  for (int i = 1; i <= light_count; i++) {
	...
	int SSAA = luaL_checknumber(L, 11);
	int softShadowFactor = luaL_checknumber(L, 12);
	int distributionFactor = luaL_checknumber(L, 13);

	double camera[3];
	get_tuple(L, 14, camera, 3);

  a4_render(root->node, filename, width, height,
            eye, view, up, fov,
            ambient, lights, SSAA, softShadowFactor, 
						distributionFactor, camera[0], camera[1], camera[2]);
...
extern "C"
int gr_trans_material_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  double blur = luaL_checknumber(L, 4);
  
  data->material = new TransparentMaterial(Colour(kd[0], kd[1], kd[2]), refract, opacity, blur);
...
extern "C"
int gr_node_add_operation_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  luaL_argcheck(L, childdata != 0, 2, "Node expected");
	...
	const char* operation = luaL_checkstring(L, 3);

  SceneNode* child = childdata->node;

  self->add_csg(child, operation);
...
static const luaL_reg grlib_functions[] = {
  ...
  {"cyllinder", gr_cyllinder_cmd},
  {"cone", gr_cone_cmd},
  {"joint", gr_joint_cmd},
  {"phongMat", gr_phong_material_cmd},
  {"transMat", gr_trans_material_cmd},
