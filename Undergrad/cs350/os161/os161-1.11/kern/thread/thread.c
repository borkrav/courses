...
mi_switch(threadstate_t nextstate)
{
	...
	struct addrspace *oldSpace = cur->t_vmspace;
	...
	struct addrspace *newSpace = curthread->t_vmspace;
...
thread_wakeup_one(const void *addr)
{
	...
	if(array_getnum(sleepers) > 0){
		do{
			t = (struct thread *)array_getguy(sleepers, i);
		}while( (t->t_sleepaddr !=addr) && ++i < array_getnum(sleepers));
		
		if( i < array_getnum(sleepers)){
			array_remove(sleepers, i);
		
			assert(make_runnable(t)==0);	
		}
	}
...
int
thread_add_file(const char *filename, int flags)
{
	struct filehandle *newhandle = kmalloc(sizeof(struct filehandle));
	if(newhandle == NULL) return -1;
	
	char *temp = kstrdup(filename);
	if(vfs_open(temp, flags, &(newhandle->file))) return -1;
	kfree(temp);

	newhandle->offset = 0;

	int fd = sarray_getnextposition(curthread->files);

	if(sarray_add(curthread->files, newhandle)) return -1;
...
struct filehandle *
thread_obtain_file(int fd)
{
	return (struct filehandle *)sarray_getguy(curthread->files, (fd-3));
}
...
{
	kfree(thread_obtain_file(fd));
	return sarray_remove(curthread->files, (fd-3));
}
