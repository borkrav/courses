...
//Cond ints
// 1 => cats eating
// 0 => creatures finishing
// -1 => mice eating
int eatingCond = -1;


//Synch Objs
struct lock *catMutex;
struct cv *miceNotEating;

struct lock *mouseMutex;
struct cv *catsNotEating;
	       ...
	       unsigned long catnumber)
{
  ...
  for(i=0;i<NumLoops;i++) {
    ...
    /* legal bowl numbers range from 1 to NumBowls */
 	...
 	//This cat wants to see if it hast to wait
	lock_acquire(catMutex);
	
	while((miceWaiting && eatingCond != 1) || creaturesEating >= NumBowls)
		cv_wait(miceNotEating, catMutex);//wait

	//locked by catsWaiting
	creaturesEating++;//This cat is going to be eating
	lock_release(catMutex);

	cat_eat((bowlSelection++ % NumBowls)+1);//eat

  	lock_acquire(catMutex);
	eatingCond = 0;//the first to leave will stop more from coming
	if(--creaturesEating == 0)//one less cat eating; if none are eating
	{
		//Cats are done eating, mice are allowed to eat
		eatingCond = -1;

		bowlSelection = 0;
	
		//Tell mice to eat
		if(miceWaiting) cv_broadcast(catsNotEating, catMutex);
		else cv_broadcast(miceNotEating, catMutex);
	}
	lock_release(catMutex);
          ...
          unsigned long mousenumber)
{
  ...
  for(i=0;i<NumLoops;i++) {
	...
	//This mouse wants to see if it hast to wait
	lock_acquire(mouseMutex);
	//while mice aren't ready or cats aren't done
	while((catsWaiting && eatingCond != -1) || creaturesEating >= NumBowls)
		cv_wait(catsNotEating, mouseMutex);//wait

	//locked by miceWaiting
	creaturesEating++;//This mouse is going to be eating
	lock_release(mouseMutex);

	mouse_eat((bowlSelection++ % NumBowls)+1);//eat

  	lock_acquire(mouseMutex);
	eatingCond = 0;//the first to leave will stop more from coming
	if(--creaturesEating == 0)//one less mouse eating; if none are eating
	{
		//Mice are done eating, cats are allowed to eat
		eatingCond = 1;
		...
		//Tell mice to eat
		if(catsWaiting) cv_broadcast(miceNotEating, mouseMutex);
		else cv_broadcast(catsNotEating, mouseMutex);
	 ...
	 char ** args)
{
  ...
  //My Globals init
  catMutex = lock_create("Cat_Mutex");
  mouseMutex = lock_create("Mouse_Mutex");

  miceNotEating = cv_create("Mice_Not_Eating");
  catsNotEating = cv_create("Cats_Not_Eating");
